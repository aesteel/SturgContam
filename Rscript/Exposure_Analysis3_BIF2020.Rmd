...
DataSum2496bif.raw = read.csv("outputData/Exposure_Outputdata/Bifenthrin_2020_Cleaned_FullData.csv")

DataSum96bif.raw = DataSum2496bif.raw[DataSum2496bif.raw$ExposureHrs==96,]

```
# this includes measured concentrations, input directly into data csv


## Add water temps at end of trial - not completed for 2020! Collected?
...
## Summarize to take total distance (sum moved in a trial) for each fish replicates
  # DataSum2496bif.dist = DataSum2496bif.raw %>%
  #   group_by(index, Trial, Arena, Replicate, ExposureHrs, Treatment, calcConc, Spp, Tray, RepID) %>%
  ...
  #   data.frame()
  # 
  # MeanDist0bif = DataSum2496bif.dist %>%
  #   subset(Treatment==0) %>%
  ...
  #   data.frame()
  # 
  # DataSumDiffbif.dist = merge(DataSum2496bif.dist, MeanDist0bif, all.x=T)
  # DataSumDiffbif.dist$DistDiff = DataSumDiffbif.dist$TotalfishDist_m - DataSumDiffbif.dist$mean0_dist


## Summarize to take total distance (sum moved in a trial) for each fish replicates
DataSum96bif.dist = DataSum96bif.raw %>%
...
```
```{r plots from Analysis2}
# classic veridis box plots
totfishdist96Bif = ggplot(DataSum96bif.dist, 
...
DataSum96bif.distGS = DataSum96bif.dist[DataSum96bif.dist$Spp=="GS",]
DataSum96bif.distWS = DataSum96bif.dist[DataSum96bif.dist$Spp=="WS",]

dist.lmGSbif = lm(TotalfishDist_m ~ lnCalcConc + lnCalcConc2,
               data=DataSum96bif.distGS, na.action=na.fail) 
 summary(dist.lmGSbif) 
 plot(dist.lmGSbif) 
 dredge(dist.lmGSbif)
  # linear model is best, not polynomial

dist.lmWSbif = lm(TotalfishDist_m ~ lnCalcConc + lnCalcConc2,
               data=DataSum96bif.distWS, na.action=na.fail) 
 summary(dist.lmWSbif) 
 plot(dist.lmWSbif) 
 dredge(dist.lmWSbif)
  # polynomial model is best

 
 # basic linear models with treatment as factor, to provide significance for boxplots
dist.lmfactGS.bif20 = lm(TotalfishDist_m ~ factor(Treatment),#lnCalcConc, # 
               data=DataSum96bif.distGS)
dist.aovGS = summary(aov(dist.lmfactGS.bif20))
dist.aovGS
 TukeyHSD(aov(dist.lmfactGS.bif20)) # 2000 is just ABRELY significant (0.046)
 
dist.lmfactWS = lm(TotalfishDist_m ~ factor(Treatment),#lnCalcConc, # 
               data=DataSum96bif.distWS)
dist.aovWS = summary(aov(dist.lmfactWS))
dist.aovWS
 TukeyHSD(aov(dist.lmfactWS)) # 2000 is sig dif from all others except 1000
...
```{r model predictions}
preddat.WSbif = data.frame(lnCalcConc= unique(DataSum96bif.distWS$lnCalcConc))
preddat.WSbif$lnCalcConc2 = preddat.WSbif$lnCalcConc^2
preddat.WSbif = preddat.WSbif[order(preddat.WSbif$lnCalcConc),]
                                           
WSpreds.bif = predict(dist.lmWSbif, newdata=preddat.WSbif, se.fit=TRUE) 
preddat.WSbif$predTotalDist_m = WSpreds.bif$fit
preddat.WSbif$predSE_TotalDist_m = WSpreds.bif$se.fit
  preddat.WSbif$spp="WS"

                
                
preddat.GSbif = data.frame(lnCalcConc= unique(DataSum96bif.distGS$lnCalcConc))
preddat.GSbif$lnCalcConc2 = preddat.GSbif$lnCalcConc^2
preddat.GSbif = preddat.GSbif[order(preddat.GSbif$lnCalcConc),]
                        
GSpreds.bif = predict(dist.lmGSbif, newdata=preddat.GSbif, se.fit=TRUE) 
preddat.GSbif$predTotalDist_m = GSpreds.bif$fit
preddat.GSbif$predSE_TotalDist_m = GSpreds.bif$se.fit
  preddat.GSbif$spp="GS"
  
 preddat.bif = rbind(preddat.GSbif, preddat.WSbif)

 # plot model predictions with raw conc scale
ggplot(preddat.bif, aes(x=exp(lnCalcConc), y=predTotalDist_m, fill=spp)) +
...
ggplot(preddat.bif, aes(x=(lnCalcConc), y=predTotalDist_m, fill=spp)) +
  geom_errorbar(aes(ymin = predTotalDist_m - predSE_TotalDist_m, 
                    ymax = predTotalDist_m + predSE_TotalDist_m)) + 
...
preddat2.bif = merge(preddat.bif,
                     DataSum96bif.dist[,c("Spp","lnCalcConc","calcConc","Treatment")], 
                     all.x=T, by.x=c("spp","lnCalcConc"), by.y=c("Spp","lnCalcConc"))
preddat2.bif= rename(preddat2.bif, Spp=spp)

totfishdist96bif.pred = ggplot(DataSum96bif.dist, 
                            aes(fill=factor(Treatment), y=TotalfishDist_m, 
                               x=factor(Treatment)) ) + 
                      geom_boxplot(outlier.size = .75) + 
                      geom_errorbar(data=preddat2.bif, 
                                    aes(x=factor(Treatment), y=predTotalDist_m,
                                        ymin = predTotalDist_m - predSE_TotalDist_m, 
                                        ymax = predTotalDist_m + predSE_TotalDist_m),
                                    color="grey30", width=.2, size=.3) + 
                      geom_point(data=preddat2.bif, 
                                 aes(x=factor(Treatment), y=predTotalDist_m), 
                      ...
                      theme_bw()
totfishdist96bif.pred


tiff("figures/Exposure_OutputFigs/TotalDistance_Bif2020_GSWS_withPreds.tiff", width=140, height=70, units="mm", compression="lzw", res=150)
totfishdist96bif.pred
dev.off()        
...
```{r quick frequentist model sig}       
# are the estimates different from control?      
 summary(dist.lmGSbif)
  summary(aov(dist.lmGSbif))
 # none of the concentrations are significantly different for GS, but close
 trashGS = lm(TotalfishDist_m ~ factor(lnCalcConc) + factor(lnCalcConc2),
               data=DataSum96bif.distGS) 
    summary(trashGS) 
    plot(trashGS)
      TukeyHSD(aov(trashGS))
      # most (only) significant is control vs 2000 (p=0.046)
      
 summary(dist.lmWSbif)
  summary(aov(dist.lmWSbif))
  
  trashWS = lm(TotalfishDist_m ~ factor(lnCalcConc) + factor(lnCalcConc2),
               data=DataSum96bif.distWS) 
    summary(trashWS) 
    plot(trashWS)
      TukeyHSD(aov(trashWS))
  # if this is a legit way to model it, 2000 is different from all other levels
...
DistPred_orig_plot_96 = ggplot() + 
  geom_point(data=DataSum2496.dist[DataSum2496.dist$ExposureHrs==96,],
             aes(x=(calcConc),                          
                 y=TotalfishDist_m, 
                 color=factor(Spp))) + 
  geom_ribbon(data = preddat.df[preddat.df$ExposureHrs==96,], aes(x=(CalcConc), 
              ymin=link_PI05, ymax=link_PI95), 
              alpha=.4, col="grey70")+
  geom_line(data = preddat.df[preddat.df$ExposureHrs==96,], aes(x=(CalcConc), y=link_mu_mn), lwd=.2)+
