...
DataSum2496.meand = DataSum2496 %>%
  group_by(index, Trial, Arena, Replicate, ExposureHrs, Treatment, calcConc, Spp, RepID) %>%
  summarize(MnMeander = mean(Meander, na.rm=T), MnTurnAngle = mean(TurnAngle, na.rm=T)) %>%
  ungroup()  %>%
...
DSmeand_bmoddat = DataSum2496.meand[,c("MnMeander","lnCalcConcC","lnCalcConcC2",
                                     "ExposureHrs","Spp","RepID")]
...
DSmeand_bmoddat$SppDummy <- ifelse(DSmeand_bmoddat$Spp=="GS", 0,1)
DSmeand_bmoddat$ExposeDummy <- ifelse(DSmeand_bmoddat$ExposureHrs=="24", 0,1)
...
compare(meandspp.blm_int1, meandspp.blm_int2, meandspp.blm_int3, meandspp.blm_int4, meandspp.blm_int5, meandspp.blm_int6)
```
...
meandspp.blm_int2.rise = map2stan(
  alist(
    MnMeander ~ dnorm(mu, sigma),  # data don't look normal; bimodal
    mu  <- a_fish[RepID] +
           be_fish[RepID]*ExposeDummy +
...
meandspp.blm_int5.rise.2 = map2stan(
  alist(
    MnMeander ~ dnorm(mu, sigma),
    mu  <- a_fish[RepID] + 
           be_fish[RepID]*ExposeDummy + 
...
meandspp.blm_int5.rise.2.offset = map2stan(
  alist(
    MnMeander ~ dnorm(mu, sigma),
    mu  <- a + a_fish[RepID] +
           be + be_fish[RepID]*ExposeDummy +
