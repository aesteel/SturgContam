...
DataSumAll.T = merge(DataSumAll, tempdat, by.x=c("Spp","ExposureHrs","Tray","Replicate"), by.y=c("species","Exposure.Hr","Tray","rep"), all.x=T)
 # this uses the control as a sample for all of the corresponding replicate numbers
```
...
DataSum96$calcConc = DataSum96$Treatment ## temporary place holder until we get the results on conc back from Luann
...
## Summarize to take total distance (sum moved in a trial) for each fish replicate
DataSum2496$calcConc = DataSum2496$Treatment ## temporary place holder until we get the results on conc back from Luann
...
DataSumAll$calcConc = DataSumAll$Treatment ## temporary place holder until we get the results on conc back from Luann
DataSumAll.dist = DataSumAll %>%
  group_by(index, Trial, Arena, Replicate, ExposureHrs, Treatment, Tray, calcConc, Spp, RepID) %>%
  #summarize(TotalfishDist = sum(SumDistMoved, na.rm=T)) %>%
  summarize(TotalfishDist_m = sum(SumDistMoved, na.rm=T)/(10*100)) %>%
  ungroup()  %>%
...
### with all four timepoints
    ...
    totfishdistAll = ggplot(DataSumAll.dist, aes(fill=factor(Treatment), y=TotalfishDist_m, 
                                   x=factor(ExposureHrs) ) ) + 
                          geom_boxplot(outlier.size = .75) + 
                          facet_wrap(~Spp, labeller = labeller(
                            Spp = c("GS"="Green Sturgeon","WS"="White Sturgeon")),#)+
