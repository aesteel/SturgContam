...
DataSum2496.meand = DataSum2496 %>%
  ...
  summarize(MnMeander = mean(Meander_degmin, na.rm=T), MnTurnAngle = mean(TurnAngle_deg, na.rm=T)) %>%
  ungroup()  %>%
  data.frame()
...
Meanderraw = ggplot(DataSum2496, aes(fill=factor(Treatment)))+ #, y=Meander_degmin, x=factor(Treatment) ) ) + 
  #geom_violin(outlier.size = .75) + 
  geom_histogram(aes(x=Meander_degmin))+#, y=factor(Treatment)) ) + 
  facet_wrap(~Treatment) +
  ylab("Frequency") + 
  xlab ("Degrees/minute (0 degrees = straight)") + 
  scale_fill_viridis_d(name="Nominal Bifenthrin\nConcentration (ng/L)") 
...
DataSum2496.mraw$TurnAngle_deg2 = DataSum2496.mraw$TurnAngle_deg
DataSum2496.mraw[DataSum2496.mraw$TurnAngle_deg<0,"TurnAngle_deg2"] <-  DataSum2496.mraw[DataSum2496.mraw$TurnAngle_deg<0,"TurnAngle_deg"] + 360 

TurnAngleraw = ggplot(DataSum2496.mraw, aes(fill=factor(Treatment)))+ #, y=Meander_degmin, x=factor(Treatment) ) ) + 
  #geom_violin(outlier.size = .75) + 
  geom_histogram(aes(x=TurnAngle_deg2))+#, y=factor(Treatment)) ) + 
  facet_wrap(~Treatment) +
  ylab("Frequency") + 
  xlab ("Degrees (degrees to R)") + 
  scale_fill_viridis_d(name="Nominal Bifenthrin\nConcentration (ng/L)") 
