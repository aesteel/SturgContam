...
library(CircStats) # for turn angle analysis
...
#### Read in metadata 
```{r read metadata}
# used this code to write from .xlsx to .csv files when I accidentally exported them in the wrong format
          # library(readxl)
          # 
          # setwd("/Volumes/5TB Contam/2020 Bif Contam Pilot/Ethovision/Exposure_WS_Pilot/Export Files/MDM2 02 75 mm AbsRelTA")
          # 
          # listxlsx = dir(pattern="xlsx")
          # 
          # genericnames = excel_sheets(listxlsx[1])
          # counter=0
          # 
          # for(i in 1:length(listxlsx)) {
          #   for(j in 1:length(genericnames)) {
          #     counter = counter+1
          #     print(counter)
          #       workbookij = read_excel(listxlsx[i], sheet=genericnames[j])
          #       csvfilename = paste0("Track-",substr(listxlsx[i], 10, 38),
          #                                substr(genericnames[j], 6,18), ".csv")
          #       write.csv(workbookij, csvfilename, row.names=F)
          #   }
          # }
...
WSfilelist = list.files(path="/Users/Anna/Documents/ResearchGit/SturgContam/rawData/Exposure_EthoExport_Complete_WS2020/MDM2_02_75mmAbsRelTA", pattern=".txt") 

# WSfilelist = list.files(path="/Volumes/5TB Contam/2020 Bif Contam Pilot/Ethovision/Exposure_WS_Pilot/Export Files/MDM2 02 75 mm AbsRelTA", pattern=".txt")
...
# set directory where .txt files are for each species
    ...
    # WSdatadir = "/Volumes/5TB Contam/2020 Bif Contam Pilot/Ethovision/Exposure_WS_Pilot/Export Files/MDM2 02 75 mm AbsRelTA/"
...
# do.call function to use above function to read and then rbind all the trial datafiles into one df per species    
  ...
  # Tried to merge these datafiles here, but the result is too large to work with; will combine after reduce through summarizing by time (into intervals of 0.2 seconds)
...
GSdatasum = GSdatasub %>%
  ...
  summarize(TrialTime = round(mean(TrialTime_s),1),
            ...
            TurnAngleCirc = deg(circ.mean.na(rad(TurnAngle_degrel2))),
            InZoneB = max(InZoneB), 
            InZoneC = max(InZoneC) ) %>%
  ungroup() %>%
  data.frame()
...
#WSdatasub$TurnAngle_degrel2rad = rad(WSdatasub$TurnAngle_degrel2)
