...
veldat_maxXcm = function(burst) {
  ...
  vel_dat_2 <- segment_dat_1i %>% ## complete the dataframe with velocities for each burst event
    ...
    mutate(SEG_VELOCITY = DIFF/TIME_DIFF) %>% 
    ...
    filter(SEG_VELOCITY >= 0) ## should this be filtered our elsewhere?
  return(vel_dat_2)
}
...
nn=3
burst.mntopvels = do.call(rbind, lapply(burst.df.list, topvel_func, n=nn))
...
tiff(filename=paste0(figureoutputfolder,"/Subjective_Rank_Mn_dropTop2_avgNext",nn,"_",filestart[f],".tiff"), width=83, height=75, res=300, units="mm")
subjective_rank_spds
dev.off()


mn.burstvalue = mean(burst.mntopvels$mntopvel[burst.mntopvels$burstQual=="good"])
sd.burstvalue = sd(burst.mntopvels$mntopvel[burst.mntopvels$burstQual=="good"])
