...
fish.data <- read.csv("../rawData/Metabolism_GSWSfip/fishdata_metabolism_gs_ws_2021.csv", fileEncoding="UTF-8-BOM")
...
RMR_raw <- readclean("../rawData/Metabolism_GSWSfip/GSfip_rawdata/5_30_21_raw.txt")
...
RMR_raw <- readclean("../rawData/Metabolism_GSWSfip/GSfip_rawdata/5_30_21_round2_raw.txt")
...
RMR_raw <- readclean("../rawData/Metabolism_GSWSfip/GSfip_rawdata/5_31_21_raw.txt")
...
RMR_raw <- readclean("../rawData/Metabolism_GSWSfip/GSfip_rawdata/5_31_21_round2_raw.txt")
...
RMR_raw <- readclean("../rawData/Metabolism_GSWSfip/GSfip_rawdata/6_1_21_raw.txt")
...
RMR_raw <- readclean("../rawData/Metabolism_GSWSfip/GSfip_rawdata/6_1_21_round2_raw.txt")
...
RMR_raw <- readclean("../rawData/Metabolism_GSWSfip/GSfip_rawdata/6_1_21_round3_raw.txt")
...
RMR_raw <- readclean("../rawData/Metabolism_GSWSfip/GSfip_rawdata/6_2_21_raw.txt")
...
RMR_raw <- readclean("../rawData/Metabolism_GSWSfip/GSfip_rawdata/6_2_21_round2_raw.txt")
...
RMR_raw <- readclean("../rawData/Metabolism_GSWSfip/GSfip_rawdata/6_2_21_round3_raw.txt")
...
RMR_raw <- readclean("../rawData/Metabolism_GSWSfip/GSfip_rawdata/6_3_21_raw.txt")
...
RMR_raw <- readclean("../rawData/Metabolism_GSWSfip/GSfip_rawdata/6_3_21_round2_raw.txt")
...
RMR_raw <- readclean("../rawData/Metabolism_GSWSfip/GSfip_rawdata/6_3_21_round3_raw.txt")
...
RMR_raw <- readclean("../rawData/Metabolism_GSWSfip/GSfip_rawdata/6_4_21_raw.txt")
...
RMR_raw <- readclean("../rawData/Metabolism_GSWSfip/GSfip_rawdata/6_4_21_round2_raw.txt")
...
### GS data tidying, visualization, and analysis
```{r GS add chem analysis to dataframe}
# add measured concentrations to the dataframe
fipGS_chemanalysis = read.csv("../rawData/Metabolism_GSWSfip/ChemAnalysis_GSFipronil_2021_Reduced.csv")
...
O2.all.temp$date = as.Date(substring(O2.all.temp$date,3,10), format="%y-%m-%d")

#reorganize_names2 = c("DateTime","spp","Contam","Temp","NomConc", "CalcConc","fishID", "weight.g","length.mm", "round", "chamber", "volume.ml", "BaroP", "Salinity","Phase", "Time.m", "Temp", "O2.sat", "Intercept", "Slope", "Slope.hour", "r2", "sat100.mgO2perL", "sat100.mgO2", "mgO2", "mgO2hpos", "mass.specific.mgO2ghr", "notes")
```

```{r GS calc lowMO2}


#write.csv(O2.all.temp, "../outputData/Metabolism_GSWSfip/GS2021Fip_All.O2slope.Measures.csv")

O2.all.temp = read.csv("../outputData/Metabolism_GSWSfip/GS2021Fip_All.O2slope.Measures.csv")


GS21.O2.all = O2.all.temp

hist(GS21.O2.all$r2)
hist(filter(GS21.O2.all, r2<0.90)$r2)

# fix Date
GS21.O2.all$date = as.Date(GS21.O2.all$DateTime)

# remove pink and blue (for now?) because the chem analysis was bad for those two treatments
GS21.O2.all <- GS21.O2.all[!(GS21.O2.all$treatment %in% c("blue","pink")),]

# filter out poor r2
GS21.O2.all.r2filt <- filter(GS21.O2.all, r2 > 0.98)
 # removes 288 segments, and 10 unique fish all together (ID in 'poorR2_ID' next)
 poorR2_ID = unique(GS21.O2.all$fishID)[is.na(match(unique(GS21.O2.all$fishID), 
                                                    unique(GS21.O2.all.r2filt$fishID)) )]

# identify fish with <3 measurements after removing poor r2 (fish not removed yet)
 SampNumb = data.frame(table(GS21.O2.all.r2filt$fishID))
 lowSamp = SampNumb[SampNumb$Freq<3,]
...
sort.GS21.O2 <- GS21.O2.all.r2filt[order(GS21.O2.all.r2filt$fishID,
                                    GS21.O2.all.r2filt$mass.specific.mgO2ghr),]

lowest3.GS21 <- by(sort.GS21.O2, sort.GS21.O2["fishID"], head, n=3) # creates a list of lowest three
low3O2.GS21 <- Reduce(rbind, lowest3.GS21) # merges the list back into a dataframe

# check that IDs with <3 good traces are the same as 'lowSamp'
data.frame(table(low3O2.GS21$fishID))[as.data.frame(table(low3O2.GS21$fishID))$Freq<3,]

# reduce dataset to only those with 3 metabolism traces with r2 over 0.98
low3MO2.GS21 = low3O2.GS21[!(low3O2.GS21$fishID %in% lowSamp$Var1),]

# set treatment and measurement levels to factor
  low3MO2.GS21$treatment = factor(low3MO2.GS21$treatment,
                                  levels=c("white","green","yellow","red"))
  low3MO2.GS21$Phase = factor(low3MO2.GS21$Phase,
                                  levels=paste0("M",1:15))
  
    
# 63 unique fish remain
# remaining sample sizes per treatment if remove all those with < 3 traces that have r2>0.98 and are not in pink or blue
  table(unique(low3MO2.GS21[,c("fishID","treatment")])$treatment)
...
write.csv(O2.GS21.final, "../outputData/GS2021Fip_Lowest3.O2slope.Measures.csv")
...
ggplot(low3MO2.GS21, aes(x= Phase, y = mass.specific.mgO2ghr, fill = treatment))+
  geom_jitter(width=.1, pch=21, size=3) + 
  scale_fill_manual(values=c("grey90","green3","goldenrod1","red")) + 
  theme_bw()

ggplot(low3MO2.GS21, aes(x= chamber, y = mass.specific.mgO2ghr, fill = treatment))+
  geom_jitter(width=.1, pch=21, size=3) + 
  scale_fill_manual(values=c("grey90","green3","goldenrod1","red")) +
  facet_wrap(.~date) + 
    theme_bw()

ggplot(low3MO2.GS21, aes(x= round, y = mass.specific.mgO2ghr, color = log(CalcConc+1)))+
  geom_jitter(width=.3) +  facet_wrap(.~date) + 
    theme_bw()

# trends in mean by relevant experimental conditions
ggplot(O2.GS21.final, aes(x= chamber, y = mn3.mass.spec.MO2, color = log(CalcConc+1)))+
  geom_jitter(width=.1) + facet_wrap(.~date) + 
    theme_bw()

ggplot(O2.GS21.final, aes(x= round, y = mn3.mass.spec.MO2, fill = factor(CalcConc)))+
...
```{r GS output plots and models}

finalplot.MO2.GS = ggplot(O2.GS21.final, aes(x = as.factor(CalcConc), y = mn3.mass.spec.MO2))+
  ...
  theme_bw() + coord_cartesian(ylim=c(0,.65))
finalplot.MO2.GS + xlab("Fipronil\nNominal Exposure Concentration (ug/L)")




mlm.gs.0a = lmer(mn3.mass.spec.MO2 ~ 1 + (1|chamber), data = O2.GS21.final)
 summary(mlm.gs.0a) # chamber explains about half of the variance
 ranef(mlm.gs.0a)  # chambers 2,4,5,6 are low, 1,3,7,8 are high
 
  mlm.gs.0b = lmer(mn3.mass.spec.MO2 ~ 1 + (1|round), data = O2.GS21.final) # singluar fit
   summary(mlm.gs.0b) # round has singular fit, no variance explained
  # means of the MO2 for the three groups are very simliar: rd1=.351, rd2=.373, rd3=.353
  
  mlm.gs.0c = lmer(mn3.mass.spec.MO2 ~ 1 + (1|date), data = O2.GS21.final) # singluar fit
   summary(mlm.gs.0c) # date has singular fit, no variance explained
  # means of the MO2 for the six groups are simliar with no directional trend

   
mlm.gs.1 = lmer(mn3.mass.spec.MO2 ~ log(CalcConc+1) + (1|chamber), data = O2.GS21.final)
 summary(mlm.gs.1) # tiny effect of concentration, when compared to intercept
 ranef(mlm.gs.1)   # greater effect of chamber than concentration
 #anova(mlm.gs.1)
 # specialized mixed model anova test:
 afex::mixed(mn3.mass.spec.MO2 ~ log(CalcConc+1) + (1|chamber), data = O2.GS21.final)
  # p = 0.264, estDF = 55.87
   
 
# mlm.gs.2 = lmer(mn3.mass.spec.MO2 ~ log(CalcConc+1)  + (1|round), data = O2.GS21.final)
#  summary(mlm.gs.2)
#  ranef(mlm.gs.2)
 
 AIC(mlm.gs.1) - AIC(mlm.gs.0a)
  # delta AIC = 9.367; null model with lower AIC than full model
 
m.gs.1 = lm(mn3.mass.spec.MO2 ~ log(CalcConc+1) , data = O2.GS21.final, na.action="na.fail")
 summary(m.gs.1)
 summary(aov(m.gs.1))
  # in agreement with mixed model; tiny effect of calcConc, not statistically significant
  dredge(m.gs.1) # best model is without calcConc; deltaAIC = 1.81 
...
RMR_raw <- readclean("../rawData/Metabolism_GSWSfip/WSfip_rawdata/WS_6_28_21_raw.txt")
...
RMR_raw <- readclean("../rawData/Metabolism_GSWSfip/WSfip_rawdata/WS_6_28_21_round2_raw.txt")
...
RMR_raw <- readclean("../rawData/Metabolism_GSWSfip/WSfip_rawdata/WS_6_28_21_round3_raw.txt")
...
RMR_raw <- readclean("../rawData/Metabolism_GSWSfip/WSfip_rawdata/WS_6_29_21_raw.txt")
...
RMR_raw <- readclean("../rawData/Metabolism_GSWSfip/WSfip_rawdata/WS_6_29_21_round2_raw.txt")
...
RMR_raw <- readclean("../rawData/Metabolism_GSWSfip/WSfip_rawdata/WS_6_29_21_round3_raw.txt")
...
RMR_raw <- readclean("../rawData/Metabolism_GSWSfip/WSfip_rawdata/WS_6_30_21_raw.txt")
...
RMR_raw <- readclean("../rawData/Metabolism_GSWSfip/WSfip_rawdata/WS_6_30_21_round2_raw.txt")
...
RMR_raw <- readclean("../rawData/Metabolism_GSWSfip/WSfip_rawdata/WS_6_30_21_round3_raw.txt")
...
RMR_raw <- readclean("../rawData/Metabolism_GSWSfip/WSfip_rawdata/WS_7_1_21_raw.txt")
...
RMR_raw <- readclean("../rawData/Metabolism_GSWSfip/WSfip_rawdata/WS_7_1_21_round2_raw.txt")
...
RMR_raw <- readclean("../rawData/Metabolism_GSWSfip/WSfip_rawdata/WS_7_1_21_round3_raw.txt")
...
RMR_raw <- readclean("../rawData/Metabolism_GSWSfip/WSfip_rawdata/WS_7_2_21_raw.txt")
...
RMR_raw <- readclean("../rawData/Metabolism_GSWSfip/WSfip_rawdata/WS_7_2_21_round2_raw.txt")
...
RMR_raw <- readclean("../rawData/Metabolism_GSWSfip/WSfip_rawdata/WS_7_2_21_round3_raw.txt")
...
# add measured concentrations to the dataframe
fipWS_chemanalysis = read.csv("../rawData/Metabolism_GSWSfip/ChemAnalysis_WSFipronil_2021_Reduced.csv")
...
WS21.O2.all.temp <- merge(O2.all, filter(fipWS_chemanalysis, Contam=="Fipronil"), all.x=T)
WS21.O2.all.temp$CalcConc = as.numeric(WS21.O2.all.temp$CalcConc)
WS21.O2.all.temp$date = as.Date(substring(WS21.O2.all.temp$date,3,10), format="%y-%m-%d")
...
write.csv(WS21.O2.all.temp, "../outputData/Metabolism_GSWSfip/WS2021Fip_All.O2slope.Measures.csv")

WS21.O2.all.temp = read.csv("../outputData/Metabolism_GSWSfip/WS2021Fip_All.O2slope.Measures.csv")


WS21.O2.all = WS21.O2.all.temp

hist(WS21.O2.all$r2)
hist(filter(WS21.O2.all, r2<0.90)$r2) # there are~150 with 0-0.2 r2; not like this for GS

# fix Date
WS21.O2.all$date = as.Date(WS21.O2.all$DateTime)
# fix fishID
WS21.O2.all$fishID = as.character(WS21.O2.all$fishID)


# remove pink and blue (for now?) because the chem analysis was bad for those two treatments
WS21.O2.all <- WS21.O2.all[!(WS21.O2.all$treatment %in% c("blue","pink")),]

# filter out poor r2
WS21.O2.all.r2filt <- filter(WS21.O2.all, r2 > 0.98)
 # removes 185 segments, but only 1 unique fish  (ID in 'poorR2_ID' next)
 poorR2_ID = unique(WS21.O2.all$fishID)[is.na(match(unique(WS21.O2.all$fishID), 
                                                    unique(WS21.O2.all.r2filt$fishID)) )]

# identify fish with <3 measurements after removing poor r2 (fish not removed yet)
 SampNumb = data.frame(table(WS21.O2.all.r2filt$fishID))
 lowSamp = SampNumb[SampNumb$Freq<3,] # 6 more to remove here
 
# Calculate lowMO2 - keep lowest 3
# the rationale (as guessed by Anna) is that the least slope in the line (ie: the lowers O2 use per hr) corresponds with the most relaxed state of the fish which is the goal since this is an RMR not a MMR
sort.WS21.O2 <- WS21.O2.all.r2filt[order(WS21.O2.all.r2filt$fishID,
                                    WS21.O2.all.r2filt$mass.specific.mgO2ghr),]

lowest3.WS21 <- by(sort.WS21.O2, sort.WS21.O2["fishID"], head, n=3) # creates a list of lowest three
low3O2.WS21 <- Reduce(rbind, lowest3.WS21) # merges the list back into a dataframe

# check that IDs with <3 good traces are the same as 'lowSamp'
data.frame(table(low3O2.WS21$fishID))[as.data.frame(table(low3O2.WS21$fishID))$Freq<3,]

# reduce dataset to only those with 3 metabolism traces with r2 over 0.98
low3MO2.WS21 = low3O2.WS21[!(low3O2.WS21$fishID %in% lowSamp$Var1),]

# set treatment and measurement levels to factor
  low3MO2.WS21$treatment = factor(low3MO2.WS21$treatment,
                                  levels=c("white","green","yellow","red"))
  low3MO2.WS21$Phase = factor(low3MO2.WS21$Phase,
                                  levels=paste0("M",1:15))
  
    
# 73 unique fish remain
# remaining sample sizes per treatment if remove all those with < 3 traces that have r2>0.98 and are not in pink or blue
  table(unique(low3MO2.WS21[,c("fishID","treatment")])$treatment)
   # white  green yellow  red
   #  18     20    19     16
...
write.csv(O2.WS21.final, "../outputData/WS2021Fip_Lowest3.O2slope.Measures.csv")
...
#trends in lowest three measures by relevant experimental conditions
ggplot(low3MO2.WS21, aes(x= as.numeric(Phase), y = mass.specific.mgO2ghr, fill = treatment))+
    geom_smooth(aes(color=treatment), method = "lm") + 
   geom_jitter(width=.1, pch=21, size=3) + 
  scale_fill_manual(values=c("grey90","green3","goldenrod1","red")) + 
  scale_color_manual(values=c("grey90","green3","goldenrod1","red")) + 
  theme_bw()

ggplot(low3MO2.WS21, aes(x= chamber, y = mass.specific.mgO2ghr, fill = treatment))+
  geom_jitter(width=.1, pch=21, size=3) + 
  scale_fill_manual(values=c("grey90","green3","goldenrod1","red")) +
  facet_wrap(.~date) + 
    theme_bw()

ggplot(low3MO2.WS21, aes(x= round, y = mass.specific.mgO2ghr, color = log(CalcConc+1)))+
  geom_jitter(width=.3) +  facet_wrap(.~date) + 
    theme_bw()

# trends in mean by relevant experimental conditions
ggplot(O2.WS21.final, aes(x= chamber, y = mn3.mass.spec.MO2, color = log(CalcConc+1)))+
    geom_jitter(width=.1) + facet_wrap(.~date) + 
    theme_bw()

ggplot(O2.WS21.final, aes(x= as.numeric(round), y = mn3.mass.spec.MO2, fill = factor(CalcConc)))+
  geom_jitter(width=.3, pch=21, size=3) + 
  scale_fill_manual(values=c("grey90","green3","goldenrod1","red")) +
  facet_wrap(.~date) + 
    theme_bw()
...
ggplot(low3MO2.WS21, aes(x= Phase, y = mass.specific.mgO2ghr, color = fishID))+
  geom_point()+
  facet_wrap(~treatment)


finalplot.MO2.WS = ggplot(O2.WS21.final, aes(x = as.factor(CalcConc), y = mn3.mass.spec.MO2))+
  geom_boxplot(aes(fill=factor(CalcConc))) +
  scale_x_discrete(labels = c("0","1","10","100")) + 
  scale_fill_manual(guide=FALSE, values=c("grey90","green3","goldenrod1","red")) + 
  ylab("White Sturgeon\nMass Specific Metabolic Rate") + 
...
mlm.ws.0a = lmer(mn3.mass.spec.MO2 ~ 1 + (1|chamber), data = O2.WS21.final)
 ...
 ranef(mlm.ws.0a)  # chambers 2-6 are low, 1,7,8 are high (same pattern as with GS, except for chamber #3; scale of difference is smaller than with gs
  ...
  mlm.ws.0b = lmer(mn3.mass.spec.MO2 ~ 1 + (1|round), data = O2.WS21.final) # singluar fit
  ...
  mlm.ws.0c = lmer(mn3.mass.spec.MO2 ~ 1 + (1|date), data = O2.WS21.final) 
...
mlm.ws.1 = lmer(mn3.mass.spec.MO2 ~ log(CalcConc+1) + (1|chamber), data = O2.WS21.final)
 ...
 #anova(mlm.gs.1)
 # specialized mixed model anova test:
 afex::mixed(mn3.mass.spec.MO2 ~ log(CalcConc+1) + (1|chamber), data = O2.WS21.final)
  # p = 0.715, estDF = 68.53
...
m.ws.1 = lm(mn3.mass.spec.MO2 ~ log(CalcConc+1) , data = O2.WS21.final, na.action="na.fail")
 summary(m.ws.1)
 summary(aov(m.ws.1))
  # in agreement with mixed model; tiny effect of calcConc, not statistically significant
  dredge(m.ws.1) # best model is without calcConc; deltaAIC = 1.75 
...
### final stats
```{r final stats both}
O2.21.final = rbind(data.frame(O2.GS21.final, spp="GS"), data.frame(O2.WS21.final, spp="WS"))

mlm.0 = lmer(mn3.mass.spec.MO2 ~ 1 + (1|chamber), data = O2.21.final)
summary(mlm.0); ranef(mlm.0) # explains about half the variation; higher rates in outer chambers

mlm.1 = lmer(mn3.mass.spec.MO2 ~ log(CalcConc+1) + (1|chamber), data = O2.21.final)
 summary(mlm.1) # tiny effect of concentration, when compared to intercept
 ranef(mlm.1)   
 # specialized mixed model anova test:
 afex::mixed(mn3.mass.spec.MO2 ~ log(CalcConc+1) + (1|chamber), data = O2.21.final)

mlm.2 = lmer(mn3.mass.spec.MO2 ~ log(CalcConc+1) + spp + (1|chamber), data = O2.21.final)
