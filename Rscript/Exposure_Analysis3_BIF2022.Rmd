...
```{r chem analysis results}
 chemcsvtemp.raw = read.csv("//Users/Anna/Documents/ResearchGit/SturgContam/rawData/ChemAnalysis_Bifenthrin_2022_WSSummary.csv")

DataSum96temp.cc <- merge(DataSum96temp.raw, chemcsvtemp.raw, by.x=c("nomconc_ngL"), by.y=c("nomconc_ngL"), all.x=T)
  DataSum96temp.cc$calcConc[DataSum96temp.cc$calcConc=="ND"] = 0
  DataSum96temp.cc$calcConc = as.numeric(DataSum96temp.cc$calcConc)
...
### remove results for WS trial 45 (tray C, 96hr, 189C) until can review them in ethovision
```{r remove trial 45-WS}
DataSum96temp = DataSum96temp.cc[!(DataSum96temp.cc$tray=="C"),]
...
DataSum96temp.dist = DataSum96temp %>%
  group_by(Arena, rep, ExposureHrs, nomconc_ngL, calcConc, tray, temp) %>%
...
## with 96
totfishdist96.bif22 = ggplot(DataSum96temp.dist, 
...
## with 96, by temperature
viridis_6col = viridis(6)
totfishdist96.bif22.temp = ggplot(DataSum96temp.dist, 
                             aes(fill=factor(nomconc_ngL), y=TotalfishDist_m, 
                               x=factor(temp) ) ) + 
                      geom_boxplot(outlier.size = .75) + 
                      facet_grid(~nomconc_ngL, labeller = labeller(
                        nomconc_ngL = c("0"="0 ng/L bifenthrin",
                                        "10"="10 ng/L bifenthrin",
                                        "100"="100 ng/L bifenthrin",
                                        "500"="500 ng/L bifenthrin")) )+
                       # scales="free") +
                      ylab("Total Trial Distance (m)") + 
                      xlab("Water Temperature (\u00B0C)") +
                      scale_fill_manual(guide=F, values=viridis_6col[1:4]) + 
                      theme_bw()

totfishdist96.bif22.temp
...
disttemp.lmWS = lm(TotalfishDist_m ~ lnCalcConc*temp + lnCalcConc2*temp,
               data=DataSum96temp.dist, na.action=na.fail) 
 summary(disttemp.lmWS)  # no significance
 dredge(disttemp.lmWS) # best model has polynomial and temp and interaction

 
disttemp.lmWS2 = lm(TotalfishDist_m ~ lnCalcConc + lnCalcConc2*temp,
               data=DataSum96temp.dist, na.action=na.fail) 
 summary(disttemp.lmWS2)  # now lncalcConc2 and interaction with temp is significant
 dredge(disttemp.lmWS2)
...
```{r model predictions}
preddat.temp.WS = expand.grid(lnCalcConc= unique(DataSum96temp.dist$lnCalcConc), temp = c(12,15,18)) 
                        
preddat.temp.WS$lnCalcConc2 = preddat.temp.WS$lnCalcConc^2
preddat.temp.WS = preddat.temp.WS[order(preddat.temp.WS$lnCalcConc),]
                                           
WSpreds.temp = predict(disttemp.lmWS2, newdata=preddat.temp.WS, se.fit=TRUE) 

preddat.temp.WS$predTotalDist_m = WSpreds.temp$fit
preddat.temp.WS$predSE_TotalDist_m = WSpreds.temp$se.fit
  preddat.temp.WS$spp="WS"

                

 # plot model predictions with raw conc scale
ggplot(preddat.temp.WS, aes(x=exp(lnCalcConc), y=predTotalDist_m, group=temp, fill=temp)) +
...
ggplot(preddat.temp.WS, aes(x=(lnCalcConc), y=predTotalDist_m, group=temp, fill=temp)) +
  geom_errorbar(aes(ymin = predTotalDist_m - predSE_TotalDist_m, 
                    ymax = predTotalDist_m + predSE_TotalDist_m)) + 
...
treat.ref = unique(DataSum96temp.dist[,c("lnCalcConc","nomconc_ngL")])
preddat.temp.WS = merge(preddat.temp.WS, treat.ref, all.x=T)
preddat.temp.WS$calcConc = exp(preddat.temp.WS$lnCalcConc)-1 
...
totfishdist96.bif22 = ggplot(DataSum96temp.dist, 
                             aes(fill=factor(nomconc_ngL), y=TotalfishDist_m, 
                               x=factor(nomconc_ngL) ) ) + 
                      geom_boxplot(outlier.size = .75) + 
                      geom_errorbar(data=preddat.temp.WS, 
                                    aes(x=factor(nomconc_ngL), y=predTotalDist_m,
                                        ymin = predTotalDist_m - predSE_TotalDist_m, 
                                        ymax = predTotalDist_m + predSE_TotalDist_m),
                                    color="grey30", width=.2, size=.3) + 
                      geom_point(data=preddat.temp.WS, 
                                 aes(x=factor(nomconc_ngL), y=predTotalDist_m), 
                      ...
                      scale_y_continuous(name="Total Distance Traversed (cm)",
                                         breaks=seq(0,65,10),
                                         labels=seq(0,65,10)*100,
                                         limits=c(0,64)) +
                      xlab("Nominal Bifenthrin Concentration (ng/L)") +
                      #scale_fill_viridis_d(name="") +
                      scale_fill_manual(values=fourcolors, name="") + 
                      theme_bw()
...
totfishdist96.bif22.temp = ggplot(DataSum96temp.dist, 
                      ...
                      geom_boxplot(outlier.size = .75) + 
                       geom_errorbar(data=preddat.temp.WS, 
                                    aes(x=factor(temp), y=predTotalDist_m*100,
                                        ymin = predTotalDist_m*100 - predSE_TotalDist_m*100, 
                                        ymax = predTotalDist_m*100 + predSE_TotalDist_m*100),
                                    color="grey30", width=.2, size=.3) + 
                      geom_point(data=preddat.temp.WS, 
                                 aes(x=factor(temp), y=predTotalDist_m*100), 
                                 pch=21, size=3, fill="grey60", color="grey30") +
                      facet_grid(~nomconc_ngL, 
                                 labeller = labeller( nomconc_ngL = c("0"="0 ng/L bifenthrin",
                                        "10"="10 ng/L bifenthrin",
                                        "100"="100 ng/L bifenthrin",
                                        "500"="500 ng/L bifenthrin")) )+
                       # scales="free") +
                      ylab("Total Trial Distance (cm)") + ylim(c(0,6500)) + 
                      xlab("Water Temperature (\u00B0C)") +
                      scale_fill_manual(guide=F, values=viridis_6col[1:4]) + 
                      theme_bw()
